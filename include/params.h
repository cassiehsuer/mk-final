 ...
 */
const int q_boot = 13000049; // ~2^23.6, prime
...
class Param
{
    public:
        ...
        const static int B_ksk_mk = 5;

        // decomposition base of hybrid product
        const static int B_hybrid = 2;
        int shift_hybrid = 1;
        ...
        const static int h_hybrid = 32;

        // Delta scalar used in bootstrapping
        const static int half_delta_boot = q_boot/(2*t);

        // standard deviation for discrete Gaussian distribution used for lwe encryption
        // constexpr static double e_st_dev = 4.39;
        constexpr static double e_st_dev = 0.3;

        // standard deviation for discrete Gaussian distribution used for hybrid product pk
        constexpr static double hybrid_e_st_dev = 0.04;

        constexpr static double ksk_e_st_dev = 0.3;
        ...
        void init()
        {
            ...
            l_ksk_mk = int(ceil(log(double(q_base))/log(double(B_ksk_mk))));
            Nl_mk = N * l_ksk_mk;

            l_hybrid = int(ceil(log(double(q_boot))/log(double(B_hybrid))));

            for (size_t i = 0; i < B_bsk_size; i++)
